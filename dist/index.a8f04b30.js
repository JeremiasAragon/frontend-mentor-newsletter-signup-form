...
const addRemoveErrorClass = function(fieldName, action) {
    const closestFormGroup = document.getElementById(fieldName).closest(".form__group");
    if (action === "add") closestFormGroup.classList.add("form__group--error");
    if (action === "remove") closestFormGroup.classList.remove("form__group--error");
};
mailInput.addEventListener("change", function(e) {
    const fieldName = e.target.id;
    const userEmail = e.target.value;
    if (!isValidEmail(userEmail)) addRemoveErrorClass(fieldName, "add");
    else addRemoveErrorClass(fieldName, "remove");
});
form.addEventListener("submit", function(e) {
    e.preventDefault();
    // Get form data
    const [fieldName, userEmail] = [
        ...new FormData(this)
    ][0];
...
},{}]},["fTqHN","5AKj5"], "5AKj5", "parcelRequired218")
